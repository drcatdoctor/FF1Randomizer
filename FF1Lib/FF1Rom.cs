	...
	public partial class FF1Rom : NesRom
	{
		...
		public Blob GetFromBank(int bank, int address, int length)
		{
			if (bank == 0x1F)
			{
				...
				{
					throw new Exception("Address is outside bank bounds");
				}
				var offset = (bank * 0x4000) + (address - 0xC000);
			...
			else
			{
				...
				var offset = (bank * 0x4000) + (address - 0x8000);
				return Get(offset, length);
			}
		...
		{
			// This frees 54 bytes starting at 0xC265 in bank 1F, ROM offset: 7C275. FULL
			//    This rewrites the overworld input routine (ProcessOWInput) to remove an almost-every-frame check
			//    for entering the minigame ($C265-$C29B), and uses a DEC instead of a long LDA/STA sequence to
			//    handle the vehicle change pause (vehchgpause) around $C246.
			...
			/*
			 *  this is unused space -- former NasirCRC area
			 * CFD7   20 67 E9   JSR $E967    ; calls CHRLoad_Cont
			 * CFDA   20 99 EB   JSR $EB99    ; calls LoadBattleSpritePalettes
             * CFDD   A9 1E      LDA #$1E     ;
             * CFDF   4C 03 FE   JMP $FE03    ; SwapPRG_L to switch to bank 1E.
			 */
			PutInBank(0x1F, 0xCFD7, Blob.FromHex("2067E92099EBA91E4C03FE"));
			/*
			 * ...    1E   ; this is a LDA of what bank (was #BANK_PARTYGEN, $0E)
			 * C0AE   20 03 FE   JSR $FE03   ; SwapPRG_L
             * C0B1   20 00 80   JSR $8000   ; $8000 on bank 1E -- should be NewGamePartyGeneration
			 * ; COB4 is JSR NewGame_LoadStartingStats, $C76D on 1F, which swaps to a new bank.
			 */
			PutInBank(0x1F, 0xC0AD, Blob.FromHex("1E2003FE200080"));

			// change     JMP :+                        ; skip ahead to the part of the next routine that loads cursor CHR
            //     to     JMP $CFD7
